...
#parameters
path = '/Volumes/external/azuri/data/gaia/x-match/GaiaDR2xSimbad/xy/'
fnameList = path+'xyfiles.list'

fnameGaiaRoot = '/Volumes/external/azuri/data/gaia/dr2/xy/GaiaSource_'
fNameXMatchRoot = '/Volumes/external/azuri/data/gaia/x-match/GaiaDR2xSimbad/xy/GaiaXSimbad_%.6f-%.6f_%.6f-%.6f.csv'
...
def crossMatch(simbadFile, gaiaFile):
    if not os.path.isfile(simbadFile):
        print('crossMatch: simbadFile <'+simbadFile+'> not found')
    else:
        simbadData = csvFree.readCSVFile(simbadFile)
        print('simbadData.header = ',simbadData.header)
        if not os.path.isfile(gaiaFile):
            print('crossMatch: gaiaFile <'+gaiaFile+'> not found')
        else:
...
def process(fileNumber):
    ...
    if True:# not os.path.isfile(fnameOut):
        ...
        if not os.path.isfile(fname):
            ...
            if csvData.size() == 0:
                print('file <'+fname+'>: csvData.size() == 0')
            else:
                dat = crossMatch(fname, fnameGaia)
                if dat is None:
                    print('no crossMatch stars found for file <'+fname+'>')
                else:
                    ...
                    print('writing ',csvData.size(),' stars to file <'+fnameOut+'.csv>')

...
getDistFromGaia <- function(w=w, wsd=wsd, glon=glon, glat=glat, calcIfBimod=FALSE){
  #print("sourcing sphharm_basis.R")
  #source("./length_scale_model/sphharm_basis.R") # provides sphharm.basis
  #print("sphharm_basis.R loaded")
...
#  outList <- list("rEst[pc]"=rRes[1], "rLo[pc]"=rRes[2], "rHi[pc]"=rRes[3], "rLen[pc]"=rlen, "result_flag" = as.integer(rRes[4]), "modality_flag"=as.integer(rRes[5]))
#  return(outList)
  return(c(rRes[1:3], rlen, as.integer(rRes[4]), as.integer(rRes[5])))
}
